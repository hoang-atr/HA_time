...
param.dt0 = 0.002;                   % estimate spike time interval [s]
param.length0 = 0.1;                 % estimate spike time length [s] 
param.Npre0 = 2;                     % # pre sampling point 
param.Nwin0 = 10;                    % # sampling window
param.sort_dir = 'ascend';

data.index = index;
data.class_label = predict(classifier, feature);
data = estimate_spike_time_linear(data, model, param);
